 ...
 */
abstract class FdbPrescriber implements Prescriber {
    ...
    // @Override
    // public List<DrugToDrugInteraction> queryDrugInteractionsWithOtherDrugs(Drug
    // drug, Iterable<Drug> otherDrugs) {
    // try {
    ...
    // + "(SELECT DISTINCT HICL_SEQNO AS HICL1,C4.DDI_CODEX AS CODEX1 ,DDI_MONOX AS
    // MONOX1,DDI_DES "
    // + "FROM RGCNSEQ4 AS GCN " + "JOIN RADIMGC4 AS C4 ON (GCN.GCN_SEQNO =
    // C4.GCN_SEQNO) "
    // + "JOIN RADIMMA5 AS A5 ON (C4.DDI_CODEX = A5.DDI_CODEX) "
    // + "WHERE HICL_SEQNO = ?) AS Table1 " + "CROSS JOIN "
    // + "(SELECT DISTINCT HICL_SEQNO AS HICL2, DIN, LN ,GCN.GCN_SEQNO ,C4.DDI_CODEX
    // AS CODEX2 ,DDI_MONOX AS MONOX2 ,DDI_DES "
    // + "FROM RGCNSEQ4 AS GCN " + "JOIN RADIMGC4 AS C4 ON (GCN.GCN_SEQNO =
    // C4.GCN_SEQNO) "
    // + "JOIN RADIMMA5 AS A5 ON (C4.DDI_CODEX = A5.DDI_CODEX) "
    // + "JOIN RICAIDC1 AS RIC ON (RIC.GCN_SEQNO = GCN.GCN_SEQNO)" + "WHERE
    // HICL_SEQNO IN ("
    // + testers.toString() + ") AND DIN IN (" + identifiers.toString() + ")) AS
    // TABLE2 "
    ...
    // pStmtToQueryDrugToDrugInteractions.setInt(1,
    // drug.getIdByName("ingredientListIdentifier"));

    // ResultSet drugToDrugInteractionsAsRst =
    // pStmtToQueryDrugToDrugInteractions.executeQuery();
    ...
    // drugToDrugInteractionsAsRst.getString(2).trim();
    // while (currentDrug.getId() < idOfDrugInteracting &&
    // currentDrugsIterator.hasNext()) {
    // currentDrug = currentDrugsIterator.next();
    // }
    // if (currentDrug.getId() == idOfDrugInteracting) {
    // drugToDrugInteractions.add(DrugToDrugInteraction.createFdbDrugToDrugInteraction(drug,
    // currentDrug,
    ...
    // } catch (SQLException e) {
    // throw new IllegalStateException("SQL is bad for querying drug to drug
    // interactions.\n" + e.getSQLState());
    // }
    // }
    ...
    // try {
    // PreparedStatement pStmtToQueryFoodInteractions =
    // FDB_CONNECTION.prepareStatement(
    // "SELECT RESULT " + "FROM RDFIMGC0 AS DF " + "JOIN RGCNSEQ4 AS GC ON
    // (DF.GCN_SEQNO = GC.GCN_SEQNO)"
    // + "JOIN RDFIMMA0 AS DFI ON (DF.FDCDE = DFI.FDCDE) " + "WHERE GC.GCN_SEQNO =
    // ?");
    // pStmtToQueryFoodInteractions.setInt(1,
    // drug.getIdByName("clinicalFormulationId"));

    // ResultSet foodInteractionsAsRst =
    // pStmtToQueryFoodInteractions.executeQuery();
    ...
    // } catch (SQLException e) {
    // throw new IllegalStateException("SQL is bad for querying food
    // interactions.\n" + e.getSQLState());
    // }
    // }
    ...
    // PreparedStatement pStmtToQueryAllergyInteractions =
    // FDB_CONNECTION.prepareStatement(
    // "SELECT HICL_SEQNO, L1.HIC_SEQN, L1.HIC, HIC_DESC, C0.DAM_ALRGN_GRP,
    // DAM_ALRGN_GRP_DESC "
    ...
    // ")");
    // pStmtToQueryAllergyInteractions.setInt(1,
    // drug.getIdByName("ingredientListIdentifier"));

    // ResultSet allergyInteractionsAsRst =
    // pStmtToQueryAllergyInteractions.executeQuery();
    // List<DrugInteraction> allergyInteractionsAsObjects = new ArrayList<>();
    // while (allergyInteractionsAsRst.next()) {
    // DrugInteraction allergyInteraction =
    // DrugInteraction.createFdbAllergyInteraction(drug);
    // allergyInteractionsAsObjects.add(allergyInteraction);
    // }
    // return allergyInteractionsAsObjects;
    // } catch (SQLException e) {
    // throw new IllegalStateException("SQL is bad for querying allergy
    // interactions.\n" + e.getSQLState());
    // }
    // }
    ...
    // try {
    // PreparedStatement pStmtToQueryDrugsBasedOnPrefix =
    // FDB_CONNECTION.prepareStatement(
    // "SELECT TOP(100) t1.LN, t3.HICL_SEQNO, t1.GCN_SEQNO, t1.DIN, t1.IADDDTE,
    // t1.IOBSDTE, t2.MFG "
    ...
    // Drug drug = Drug.createFdbDrug(drugsAsRst.getString(1).trim(),
    // drugsAsRst.getInt(4),
